...
class Order(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "(instrument_type IN ('STOCK', 'CONVERTIBLE_BOND', 'ETF') AND commission_type IS NULL "
            "AND commission_parameter IS NULL "
            "AND commission_contract_multiplier IS NULL) OR "
            "(instrument_type NOT IN ('STOCK', 'CONVERTIBLE_BOND', 'ETF') AND commission_type IS NOT NULL "
