...
适用版本：`0.9.4`，更新日期：2026/08/23

> 状态：Active
> 读者：SDK 最终用户
> 权威范围：安装、初始化、数据更新时间和公开 API
> 不负责：数据库运维、内部路由实现和生产发布流程
> 对应代码/配置：SDK 公开 wrapper、类型声明与 Registry
> 最后验证：2026-08-23
...
将管理员提供的`ymm_data_sdk-0.9.4-py3-none-any.whl`放入当前目录。推荐使用conda或uv虚拟
环境，避免和原有环境产生冲突。
...
python -m pip install --upgrade --force-reinstall .\ymm_data_sdk-0.9.4-py3-none-any.whl
python -c "import ymm_data_sdk; print(ymm_data_sdk.__version__)"
```

最后一条命令应打印`0.9.4`。Jupyter用户升级后应重启kernel，并确认Notebook使用的是安装该
wheel的同一个Python环境。

### 2.1 从旧版本升级到0.9.4

0.9.4允许`get_price(frequency="1d")`在临时日线仍为`running`时返回已经完成收盘批次的合约；
未收盘合约为空，公开参数和返回结构不变。0.9.3新增本地批量最新tick快照接口
`current_snapshot()`，该接口读取已经落入本地ClickHouse的
行情，不调用RQDataC、不消耗RQDataC额度。0.9.0调整了更新状态API，0.9.1增加非阻塞回调，
0.9.2只增加服务端监控与License隔离：

- `get_update_status()`现在只接受必填的`method`，可传一个公开方法名或方法名列表。
- 新增`watch_update_status()`，用于在对应local接口成功更新时间推进时收到提醒。
- `watch_update_status()`返回的监听对象继续支持0.9.1新增的`listen(callback)`，主线程无需阻塞等待。
- 已删除公开的`get_update_events()`和`watch_update_events()`；用户不再处理事件ID、游标、
  事件正文或过滤条件。
- `get_all_factor_names()`、`get_latest_trading_date()`和`get_future_latest_trading_date()`均可在
  0.9.4中直接使用。
...
<!-- BEGIN GENERATED:RQDATAC_UPDATE_SCHEDULE -->
> 本节由 `scripts/documentation.py` 从更新 Registry 与调度 JSON 自动生成，请勿手工改表。

以下时间均为北京时间（`Asia/Shanghai`）。任务只有完成 fetch、原子 publish、standard verify 和状态登记后，新数据才可见；失败不会发布半批数据。夜盘任务的目标是即将开始的新交易日。

| RQDataC 接口 | 类型 | 上游确认更新时间 | 本地语义任务（最早开始） |
| --- | --- | --- | --- |
| `rq.all_instruments` | `snapshot` | 04:05、08:05、08:20、08:32、08:45、20:20、22:05 | `METADATA_HK_EARLY` 04:10；`METADATA_HK_MORNING` 08:10；`METADATA_FUTURES_PREOPEN` 08:25；`METADATA_GENERAL_PREOPEN` 08:35；`EARLY_AUCTION_CATALOG` 08:47；`NIGHT_INSTRUMENT_PREPARE` 20:25，目标为下一交易日；`METADATA_HK_FINAL` 22:10，目标为下一交易日 |
| `rq.instruments` | `snapshot` | 04:05、08:05、08:20、08:32、08:45、20:20、22:05 | `METADATA_HK_EARLY` 04:10；`METADATA_HK_MORNING` 08:10；`METADATA_FUTURES_PREOPEN` 08:25；`METADATA_GENERAL_PREOPEN` 08:35；`EARLY_AUCTION_CATALOG` 08:47；`NIGHT_INSTRUMENT_PREPARE` 20:25，目标为下一交易日；`METADATA_HK_FINAL` 22:10，目标为下一交易日 |
| `rq.get_tick_size` | `snapshot` | 04:05、08:05、08:20、08:32、08:45、20:20、22:05 | `METADATA_HK_EARLY` 04:10；`METADATA_HK_MORNING` 08:10；`METADATA_FUTURES_PREOPEN` 08:25；`METADATA_GENERAL_PREOPEN` 08:35；`EARLY_AUCTION_CATALOG` 08:47；`NIGHT_INSTRUMENT_PREPARE` 20:25，目标为下一交易日；`METADATA_HK_FINAL` 22:10，目标为下一交易日 |
| `rq.get_trading_dates` | `calendar` | 00:00 | `CALENDAR_MIDNIGHT` 00:05 |
| `rq.get_industry_mapping` | `snapshot` | 08:45 | `DAY_REFERENCE_PREPARE` 08:55 |
| `rq.get_industry_change` | `snapshot` | 08:45 | `DAY_REFERENCE_PREPARE` 08:55 |
| `rq.get_instrument_industry` | `snapshot` | 08:45 | `DAY_REFERENCE_PREPARE` 08:55 |
| `rq.convertible.get_industry` | `snapshot` | 08:45 | `DAY_REFERENCE_PREPARE` 08:55 |
| `rq.is_st_stock` | `dated` | 07:31 | `STOCK_STATUS_MORNING` 07:35 |
| `rq.is_suspended` | `dated` | 08:32、08:52、09:15 | `MARKET_SUSPENSION_PREOPEN` 08:35；`SUSPENSION_FINAL` 09:00；`SUSPENSION_OPEN_CONFIRM` 09:20 |
| `rq.convertible.is_suspended` | `dated` | 08:51、11:51 | `SUSPENSION_FINAL` 09:00；`CONVERTIBLE_SUSPENSION_MIDDAY` 11:55 |
| `rq.futures.get_continuous_contracts` | `dated` | 08:35 | `FUTURES_CONTINUOUS_PREOPEN` 08:40 |
| `rq.get_trading_periods` | `dated` | 上游未单列固定时间 | `DAY_PERIODS_FINAL` 08:52；`NIGHT_PERIODS_PREPARE` 20:30，目标为下一交易日；`NIGHT_PERIODS_FINAL` 20:55，目标为下一交易日；`TRADING_PERIODS_LATE` 23:30，目标为下一交易日 |
| `rq.get_price` | `dated` | 17:00 | `POSTCLOSE_MARKET_DATA` 17:15 |
| `rq.convertible.all_instruments` | `snapshot` | 07:30、19:30 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.convertible.instruments` | `snapshot` | 07:30、19:30 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.futures.get_contract_multiplier` | `snapshot` | 05:30、18:30、19:30 | `FUTURES_MULTIPLIER_MORNING` 05:35；`FUTURES_MULTIPLIER_EVENING_1835` 18:35；`FUTURES_REFERENCE_EVENING` 19:35 |
| `rq.futures.get_trading_parameters` | `snapshot` | 08:00、19:00、19:30、20:50、21:00 | `FUTURES_PARAMETERS_MORNING` 08:05；`FUTURES_PARAMETERS_SETTLEMENT_FINAL` 19:05；`FUTURES_REFERENCE_EVENING` 19:35；`FUTURES_PARAMETERS_NIGHT_PREOPEN` 20:55，目标为下一交易日；`FUTURES_PARAMETERS_NIGHT_OPEN` 21:05，目标为下一交易日 |
| `rq.convertible.get_conversion_price` | `snapshot` | 07:30、19:30 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.convertible.get_conversion_info` | `snapshot` | 07:50、19:50 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.convertible.get_call_info` | `snapshot` | 07:45、19:45 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.convertible.get_put_info` | `snapshot` | 07:35、19:35 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.convertible.get_cash_flow` | `snapshot` | 07:40、19:40 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.convertible.get_call_announcement` | `snapshot` | 07:05、19:05 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.convertible.get_credit_rating` | `snapshot` | 上游未单列固定时间 | `CONVERTIBLE_REFERENCE_MORNING` 08:00；`CONVERTIBLE_REFERENCE_EVENING` 20:00 |
| `rq.get_ex_factor` | `dated` | 08:10、08:30、08:45、09:00、17:10、20:10 | `STOCK_ADJUSTMENTS_PREOPEN_0815` 08:15；`STOCK_ADJUSTMENTS_PREOPEN_0835` 08:35；`DAY_REFERENCE_PREPARE` 08:55；`ADJUSTMENTS_OPEN_CONFIRM` 09:05；`POSTCLOSE_MARKET_DATA` 17:15；`STOCK_ADJUSTMENTS_NIGHT_PREOPEN` 20:15，目标为下一交易日 |
| `rq.futures.get_dominant` | `dated` | 17:20、18:20、19:20 | `FUTURES_ADJUSTMENTS_SETTLEMENT` 18:25；`FUTURES_ADJUSTMENTS_EVENING_1925` 19:25 |
| `rq.futures.get_ex_factor` | `dated` | 09:01、17:20、18:20、19:20、21:01、21:31 | `ADJUSTMENTS_OPEN_CONFIRM` 09:05；`FUTURES_ADJUSTMENTS_SETTLEMENT` 18:25；`FUTURES_ADJUSTMENTS_EVENING_1925` 19:25；`FUTURES_ADJUSTMENTS_NIGHT_OPEN` 21:05，目标为下一交易日；`FUTURES_ADJUSTMENTS_NIGHT_CONFIRM` 21:35，目标为下一交易日 |
| `rq.options.get_dominant_month` | `dated` | 17:30 | `OPTION_DOMINANT_CLOSE` 18:40 |
| `rq.options.get_dominant_month_rank2` | `dated` | 17:30 | `OPTION_DOMINANT_CLOSE` 18:40 |
| `rq.options.get_indicators` | `dated` | 18:40 | `OPTION_INDICATORS_CLOSE` 18:45 |
| `rq.convertible.get_std_discount` | `dated` | 18:20 | `CONVERTIBLE_DISCOUNT_CLOSE` 18:25 |
| `rq.options.get_contract_property` | `snapshot` | 09:02、21:02 | `OPTION_CONTRACT_PROPERTY_MORNING` 09:07；`OPTION_CONTRACT_PROPERTY_NIGHT` 21:07，目标为下一交易日 |
| `rq.convertible.get_indicators` | `dated` | 07:40、19:40、20:40 | `CONVERTIBLE_INDICATORS_MORNING` 07:45；`CONVERTIBLE_INDICATORS_EVENING` 19:45；`CONVERTIBLE_INDICATORS_FINAL` 20:45 |
| `rq.get_split` | `dated` | 08:25、19:25、23:25 | `SPLITS_PREOPEN` 08:30；`SPLITS_EVENING` 19:30；`SPLITS_FINAL` 23:30 |
| `rq.options.get_greeks` | `dated` | 18:25、20:25、23:25 | `OPTION_GREEKS_CLOSE` 18:30；`OPTION_GREEKS_EVENING` 20:30；`OPTION_GREEKS_FINAL` 23:30 |

`get_price` 的 1 分钟行情成功后会自动重算 5m、15m、30m、60m、120m 和 240m。每个公开 SDK 方法完成更新后会登记状态并产生可回放事件；消费者不需要按固定时刻猜测是否完成。
<!-- END GENERATED:RQDATAC_UPDATE_SCHEDULE -->
...
- `market` 当前仅支持 `'cn'`。
- 日线、分钟线和tick按“资产类型＋频率”共享一个最新权威完成交易日。同一维度的所有合约
  使用同一个截止日：查询日期小于等于`latest_authoritative_trading_date`时只读权威库，日期
  大于截止日时才读取live库；不会为每个合约逐日检索状态，也不会在同一维度逐行混合。
- 例如`equity/5m`的权威截止日为2026-08-09时，8月9日及以前读权威库，8月10日起读live库；
  当8月10日权威更新成功后，截止日自动推进到8月10日，8月11日自然成为新的live日期。
- `frequency='1d'`的临时日线只覆盖`equity/future/commodity_option/etf_option`。权威日线
  已成功更新时始终读取权威库；FeedHub按每个合约交易时段的最后1分钟分批生成当日live日线。
  尚未开始生成时返回兼容空结果；生成状态为`running`时返回已经收盘并完成验证的合约，尚未收盘的
  单合约为空，混合查询只包含已经完成的合约。例如普通期货可在15:00批次后出现，而15:15收盘的
  国债期货稍后出现。静默或无行情的单个合约仍可以返回空结果。
- 有效租约内（或已封存）的实时状态为`degraded`时仍返回已有数据，并发出`RuntimeWarning`；状态缺失、失败或租约
  过期时抛出`YMMDataUnavailableError`；表结构不兼容时抛出`YMMDataSchemaError`。
- 返回的DataFrame会在`df.attrs["ymm_data_sdk"]`中记录本次选源分段和警告。tick的重复
  时间戳是合法原始数据，SDK不会去重。
- `get_price_change_rate()`和`get_vwap()`自然使用上述融合日线。`futures.get_exchange_daily`、
  `futures.get_dominant_price(frequency='1d')`、`options.get_greeks(frequency='1d')`等独立
  日频接口仍只读取权威库。
...
### current_snapshot - 批量获取最新已落库tick快照

```python
snapshot = ymm_data_sdk.current_snapshot("AU2612")
print(snapshot)
print(snapshot.available, snapshot.datetime, snapshot.last)

snapshots = ymm_data_sdk.current_snapshot(
    ["000001.XSHE", "AU2612", "AU2612C800"],
)
for item in snapshots:
    print(item.to_dict())
```

该接口返回每个合约在当前有效交易日内最新一条已经成功写入本地数据库的tick。它不是FeedHub
内存中的最后一条实时回调，因此结果时间可能稍晚于交易所行情；每次调用都会重新读取行情，SDK
不会缓存快照内容。
...
| 参数 | 类型 | 说明 |
| --- | --- | --- |
| order_book_ids | str OR str list | 一个或多个合约代码；一次最多1000个唯一合约。 |
| market | str | 默认`'cn'`，当前仅支持中国内地市场。 |

#### 返回

- 单个字符串返回一个`Tick`对象；序列返回`list[Tick]`，保持输入顺序和重复项。
- `Tick`常用属性包括`order_book_id`、`datetime`、`last`、`open`、`high`、`low`、
  `volume`、`total_turnover`、`open_interest`、`num_trades`、`asks`、`ask_vols`、`bids`和
  `bid_vols`等，也可用`tick[name]`读取。
- `tick.available`表示目标交易日是否已有该合约的tick；有效合约尚无数据时返回
  `available=False`的空`Tick`，不会静默回退到上一交易日。`tick.to_dict()`返回便于打印的副本。
- 本地tick表没有保存的字段返回稳定的`NaN`或`None`，不会从日线、分钟线或其他接口推算。

选源与`get_price(..., frequency='tick')`一致：股票和ETF期权使用普通最近交易日，期货和商品期权
在北京时间20:55切换下一交易日；目标日期已经完成权威更新时读取`market_data_RQ`，否则读取
有效的`live_market_data_RQ`状态。`degraded`会返回已有数据并发出`RuntimeWarning`；状态缺失、
`running`、`failed`或租约过期时抛出`YMMDataUnavailableError`，schema不兼容时抛出
`YMMDataSchemaError`。混合批量中任一维度不可用时整次调用失败，不返回部分结果。

权威水位最多缓存30秒。若同一Python进程正在消费
`watch_update_status(method="get_price")`，收到权威更新后会立即清除该缓存；开启监听不是使用
`current_snapshot()`的必要条件。`get_update_status(method="current_snapshot")`返回`None`，因为
连续落库行情不存在单一完成时间；`watch_update_status(method="current_snapshot")`不受支持。

连续或虚拟期货代码不会调用上游进行实时主力替换；只有本地tick底表实际存在该代码时才会返回
数据。同一合约同一市场时间存在内容不同的多行时，SDK使用稳定指纹选择一行，但不承诺还原
FeedHub最后收到的回调顺序。
...
该接口只接受必填的`method`参数，参数值使用公开SDK方法名，不使用`rq.`前缀。传入一个方法名时
直接返回该方法的状态值；传入方法名列表时，返回`{method: status}`字典，并保持首次出现的顺序、
自动去除重复方法。

- local方法返回该接口所需本地数据最近一次成功发布并验证完成的入库时间，类型为带时区的
  `datetime`；尚无成功入库记录时返回`None`。直接由其他local数据派生的接口返回其依赖数据均已
  就绪的最近时间。
- proxy方法始终返回`None`作为哨兵值，表示该方法调用时直接查询上游，不存在本地入库时间；这不表示
  接口失败。
- `current_snapshot`同样返回`None`作为哨兵值，因为它读取连续落库的tick，不存在单一的更新完成
  时间；这不表示快照查询失败。
...
所有合约共享这一时间。接口不再返回交易日、dataset、运行/失败状态、live质量字段或事件历史。

SDK通过内部事件流收到新状态时会立即使本进程连接池中的选源水位缓存失效；没有
开启事件监听的进程会按最长30秒的缓存周期自动刷新。
...
该接口只接受必填的`method`，只在对应local数据成功更新且`get_update_status(method=...)`结果推进时
产出一次结果。普通local方法产出新的`datetime`；`get_price`只产出`last_updated_at`实际变化的
资产/频率行，不重复返回完整36行。proxy方法和`current_snapshot`没有可监听的本地完成状态，调用时
会抛出`ValueError`。
...
`total_turnover`；具体字段及数据可用性以服务端固定版本RQDataC的返回为准。

#### 范例

```python
daily = ymm_data_sdk.get_auction_info(
    ["688011.XSHG", "688012.XSHG"],
    start_date="2026-08-03",
    end_date="2026-08-07",
    frequency="1d",
)

minute = ymm_data_sdk.get_auction_info(
    "688012.XSHG",
...
查询本地交易日历中标记为包含夜盘的交易日。该接口是`ymm_data_sdk`扩展方法，不是
RQDataC同名接口，也不按单个期货合约判断是否开夜盘。
...
```python
night_dates = ymm_data_sdk.get_night_trading_dates(
...
此固定20:55边界用于兼容RQDataC接口语义，不读取FeedHub的准备日，也不判断某合约当天是否实际
开夜盘。
...
通过Proxy获取RQDataC当前可用的因子名称。`type=None`返回全部名称，也可传入RQDataC支持的
因子类别进行筛选，例如`'obos_indicator'`。

#### 参数

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| type | str or None | 因子类别；默认`None`，返回全部因子名称。非法类别由RQDataC抛出异常。 |
...
```python
all_names = ymm_data_sdk.get_all_factor_names()
obos_names = ymm_data_sdk.get_all_factor_names(type="obos_indicator")
```
...
| expect_df | bool | 默认为 `True`，返回 pandas DataFrame；设为 `False` 时返回 rqdatac 原有数据结构。 |
| market | str | 默认中国内地市场 `'cn'`；也可传入 `'hk'` 查询香港市场。 |
| **kwargs | Any | 透传给 rqdatac 的兼容参数；仅在对应 rqdatac 版本明确支持时使用。 |

#### 返回

`expect_df=True` 时返回 pandas DataFrame。查询日期区间时，结果通常以 `order_book_id` 和 `date` 为 MultiIndex，因子名称为列；传入多个因子时，每个因子对应一列。`expect_df=False` 时返回 rqdatac 的原有数据结构。
...
还是次次主力。规则与 rqdatac 3.5.6.1 保持一致：
